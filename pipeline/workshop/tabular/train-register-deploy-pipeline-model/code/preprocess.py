...
try:
  from sagemaker_container.beta.framework import (
    content_types,
    encoders,
    env,
    modules,
...
def input_fn(input_data, content_type):
  """Parse input data paylod
  We currently only take csv input. Since we need to process both labelled and
  unlabelled data we first determine whether the label column is present by looking at how many columns provided.
  """
...
def predict_fn(input_data, model):
  ...
  if label_column in input_data:
    ...
    # return only the set of feature
    return features
